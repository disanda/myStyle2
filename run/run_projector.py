...
def get_arg_parser():
    ...
    project_generated_images_parser = subparsers.add_parser('project_generated_images', help='Project generated images')
    ...
    project_real_images_parser = subparsers.add_parser('project_real_images', help='Project real images')
...
def project_images(G, images, name_prefix, args):
    ...
    #lpips_model = stylegan2.external_models.lpips.LPIPS_VGG16(pixel_min=args.pixel_min, pixel_max=args.pixel_max)
    ...
    for i in range(0, len(images), args.batch_size):
        ...
        snapshot_steps = set(args.num_steps - np.linspace(0, args.num_steps, args.num_snapshots, endpoint=False, dtype=int))
        ...
        print('dlatent###############')
        #print(proj.get_dlatent().shape)#[1,18,512]
        #print(proj.get_noise()[1].shape)#list:[4*4->8*8->8*8->16*16->16*16] 17个
        w = proj.get_dlatent()
        w_z = proj.get_noise()
        torch.save([w,w_z],'./w_wz_%d.pt'%i)
#----------------------------------------------------------------------------
...
def project_real_images(G, args):
    ...
    dataset = utils.ImageFolder(args.data_dir, pixel_min=args.pixel_min, pixel_max=args.pixel_max, resize=True, height=1024, width=1024)

    rnd = np.random.RandomState(args.seed)
    indices = rnd.choice(len(dataset), size=min(args.num_images, len(dataset)), replace=False)
    images = []
    for i in indices:
        data = dataset[i]
