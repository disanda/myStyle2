...
class Projector(nn.Module):
    ...
    def __init__(self,
        ...
        self.register_buffer('_dlatent_std',
            torch.sqrt(
    ...
    def _scale_for_lpips(self, data):
        if not self.lpips_size:
            return data
        scale_factor = self.lpips_size / min(data.size()[2:]) #压缩维度1024至256
        if scale_factor == 1:
            return data
        mode = 'nearest'
        if scale_factor < 1:
    ...
    def step(self, steps=1):
        ...
        for _ in range(steps):
            ...
            assert output.size() == self._job.target.size(), 'target size {} does not fit output size {} of generator'.format(self.target.size(), output.size())

            output_scaled = self._scale_for_lpips(output)

            # Main loss: LPIPS distance of output and target
            print(output_scaled.shape)
            print(self._job.target_scaled.shape)
